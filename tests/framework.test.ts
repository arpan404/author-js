...
describe("framework adapters", () => {
  test("express requireCan calls next when allowed", async () => {
    ...
    expect(calls).toEqual([{ entityType: "User", entity: "u", action: "read", resourceType: "Project", resource: "p", context: { ip: "127.0.0.1" }, mode: "backend" }, "next"]);
  });
  ...
  test("express requireCan sends 403 when denied", async () => {
    ...
    const middleware = expressRequireCan({ author, entityType: "User", entity: () => ({}), action: "read", resourceType: "Project", resource: () => ({}) });
  ...
  test("hono requireCan handles allow and deny", async () => {
    const allowed = honoRequireCan({ author: { evaluate: async () => allowedDecision }, entityType: "User", entity: () => ({}), action: "read", resourceType: "Project", resource: () => ({}) });
    const denied = honoRequireCan({ author: { evaluate: async () => deniedDecision }, entityType: "User", entity: () => ({}), action: "read", resourceType: "Project", resource: () => ({}) });
    const calls: unknown[] = [];
  ...
  test("elysia requireCan returns 403 body when denied", async () => {
    const hook = elysiaRequireCan({ author: { evaluate: async () => deniedDecision }, entityType: "User", entity: () => ({}), action: "read", resourceType: "Project", resource: () => ({}) });
  ...
  test("fastify requireCan sends only when denied", async () => {
    ...
    await fastifyRequireCan({ author: { evaluate: async () => allowedDecision }, entityType: "User", entity: () => ({}), action: "read", resourceType: "Project", resource: () => ({}) })({}, reply);
    await fastifyRequireCan({ author: { evaluate: async () => deniedDecision }, entityType: "User", entity: () => ({}), action: "read", resourceType: "Project", resource: () => ({}) })({}, reply);
  ...
  test("next assertCan and requireCan enforce backend checks", async () => {
    await expect(assertCan({ author: { evaluate: async () => allowedDecision }, entityType: "User", entity: {}, action: "read", resourceType: "Project", resource: {} })).resolves.toBe(allowedDecision);
    await expect(assertCan({ author: { evaluate: async () => deniedDecision }, entityType: "User", entity: {}, action: "read", resourceType: "Project", resource: {} })).rejects.toBeInstanceOf(AuthorizationDeniedError);

    type Req = { user: unknown; project: unknown };
    const check = nextRequireCan<Req>({ author: { evaluate: async () => allowedDecision }, entityType: "User", entity: (req) => req.user, action: "read", resourceType: "Project", resource: (req) => req.project });
