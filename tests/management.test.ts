...
describe("management API", () => {
  test("manages roles permissions and relations", async () => {
    const author = createAuthor({ entities: { User: UserEntity }, resources: { Project: ProjectResource }, policies: [] });
    ...
    await author.permissions.grant({ entityType: "User", entityId: "u1", action: "read", resourceType: "Project", resourceId: "p1", effect: "allow" });
    ...
    await author.relations.create({ subjectType: "User", subjectId: "u1", relation: "viewer", objectType: "Project", objectId: "p1" });
    await expect(author.relations.list({ subjectType: "User", subjectId: "u1" })).resolves.toHaveLength(1);
    await author.relations.delete({ subjectType: "User", subjectId: "u1", relation: "viewer", objectType: "Project", objectId: "p1" });
  ...
  test("write helpers invalidate cache when supported", async () => {
    ...
    const author = createAuthor({
      ...
      policies: [allow("role can read", async (ctx: Ctx) => {
        calls += 1;
